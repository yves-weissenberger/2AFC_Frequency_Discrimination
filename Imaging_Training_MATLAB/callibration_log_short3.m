...
idx =2;
frqs_dist = cat(2,frqs_dist,[frqs(idx)*2.^(-1/4),frqs(idx),frqs(idx)*2.^(1/4)])
...
for frq = frqs_dist
    frq
    snd = gensin(frq,3,params.sampleRate,params.edgeWin);
...
levels_stim = [57.9,61.9,58.6,58.1,57.2,57.4];
...
%levels Stim 
levels_stim = [58,59,59,61.2,60,59,57.8,53.7,59.3,59.7,61.5,62.3,61.5,63.6,63.0,64.0,60.2,61.6,61.3,60.5,59.8,58.1,51.6,56.9,58.9,59.5,58.4,52.7,54.2,57.4,57.4,55.7];
...
gainF =  @(x,cal) sqrt(10^((x-cal)/10));


for fr = frqs
    exponent = 0.01;
    eexp = gainF(75,levels_stim(ii))
    snd = gensin(fr,3,params.sampleRate,params.edgeWin);
    PsychPortAudio('FillBuffer', pahandle, eexp*snd*exponent);
    PsychPortAudio('Start', pahandle);
...
%levels_global = [60.5,61.3,61.9,62.1,62.8,60.2,59.1,55,60.4?,64.3?,62.4,62.9,61.3,???,64.3,63.3,58.8,60.8,58.8,58.0,59.6??,53.6,48.7,51.0,58.??,51.4,49.6,46.9,45.0,45.9,???,];


levels = [59.2,59.4,60.5];
