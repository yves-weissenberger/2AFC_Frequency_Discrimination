...
freqs = np.logspace(np.log10(6000),np.log10(24000),num=6)
nStims = len(freqs)
...
######################## Create Sound List Array ###########################
sndLat = []
for i in range(60):
    sndLat.append(list(np.ones(int(1+np.abs((np.random.normal(loc=7,scale=2)))))*0.8)+ 
                  [0.6 if np.random.randint(0,2)==1 else 1])
    
sndLat = np.array([entry for sublist in sndLat for entry in sublist])
####################### End Create Sound List Array ########################
...
while time.time() - start < ExpDur and rewTot <= rewTotMax:
	...
	if (time.time()-start-clickT)>ITI:
		...
		############################### Send Data
		if not rewLst:
		    rewLst.append([time.time()-start,'_'+str(['noRespR' if LR_target==0 else 'noRespL'][0])])
        ...
        if ((time.time() - start - clickT)>sndLat[trlIdx] and isPlaying==False):
            soundT = time.time() - start
            play_sound(stim)
	    firstLick = False
            sndL.append([time.time()-start,'_'+str(sndNr)])
            isPlaying = True
